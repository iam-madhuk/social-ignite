  ...
  try {
    ...
    if (isArticle) {
      ...
      if (mediaPath && fs.existsSync(mediaPath)) {
        console.log(`Uploading ${mediaType || 'media'}: ${mediaPath}`);
        // Take a screenshot of the open share box for debugging
        await page.screenshot({ path: 'debug_share_box_open.png' });

        // Wait for input to be attached (it might be hidden)
        await page.waitForSelector('input[type="file"]', { state: 'attached', timeout: 10000 });
        await page.setInputFiles('input[type="file"]', mediaPath);

        console.log('Uploading... waiting for media to process.');
        await page.waitForTimeout(5000); // Give it more time for larger files/processing
      }
